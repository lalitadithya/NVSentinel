...
# Shared settings and helpers for the local Slinky drain demo scripts.
...
# The namespace slinky-drainer owns. Its reconciler lists pods here and nowhere
# else, so the workload has to live in it for the custom drain to see anything.
SLINKY_NAMESPACE="${SLINKY_NAMESPACE:-slinky}"
WORKLOAD_NAME="${WORKLOAD_NAME:-slinky-workload}"
...
# The two plugin kustomizations, the drain-template ConfigMap and the chart's
# customDrain.namespace all name "nvsentinel" literally. Overriding NAMESPACE
# would create the deployments there while every follow-up command targeted the
# override, so refuse it rather than fail halfway through setup.
require_supported_namespace() {
    [[ "$NAMESPACE" == "nvsentinel" ]] || fail \
        "NAMESPACE is set to '$NAMESPACE', but this demo's plugin manifests and drain
template name 'nvsentinel' directly. Unset NAMESPACE, or edit
plugins/*/config/default and config/drain-template.yaml to match."
}
...
# Echoes a field of the DrainRequest's DrainComplete condition, or nothing when
# there is no DrainRequest yet. slinky-drainer records what it did here, so the
# demo reads the CR rather than scraping controller logs.
#   drain_complete_field <status|reason|message>
drain_complete_field() {
    local field=${1:?drain_complete_field requires a field name}

    kubectl get drainrequests -n "$NAMESPACE" -o json 2>/dev/null |
        jq -r --arg field "$field" \
            '.items[0].status.conditions[]? | select(.type == "DrainComplete") | .[$field] // empty'
}
