...
# Step 4: return to service. A drained node is still a node nobody can schedule
# on, so the demo is not finished until the fault clears and it comes back. A
# KIND node is never really repaired, so restarting the DCGM pod is what drops
# the injected fault here.
...
workload_running() {
    local ready
    ready="$(kubectl get deployment "$WORKLOAD_NAME" -n "$SLINKY_NAMESPACE" \
...
repair_gpu() {
    ...
    log "Restarting ${DCGM_NAMESPACE}/${pod}, which drops the injected fault as a real repair would have."
...
Check the pods:  kubectl get pods -n $SLINKY_NAMESPACE"
