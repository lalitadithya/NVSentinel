...
#
#   ./demo.sh            build the cluster, break the GPU, watch the custom drain run
...
usage() {
    cat <<EOF
NVSentinel local Slinky drain demo
...
run_all() {
    ...
    section "NVSentinel local Slinky drain demo"
    log "Build a cluster, break a GPU, and watch NVSentinel hand the drain to an external scheduler instead of evicting the pods itself."
