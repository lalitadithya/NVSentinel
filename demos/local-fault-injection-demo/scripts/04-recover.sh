...
has_no_gpu_condition() {
    ...
    failing="$(kubectl get node "$NODE" -o json |
        jq -r '.status.conditions[] | select(.type | startswith("Gpu")) | select(.status == "True") | .type')"
    [[ -z "$failing" ]]
}

not_cordoned() { ! is_cordoned "$NODE"; }
...
repair_gpu() {
    ...
    kubectl wait --for=condition=ready pod -l app=nvidia-dcgm \
        -n "$DCGM_NAMESPACE" --timeout=180s >/dev/null ||
        fail "The replacement DCGM pod did not become ready."
