...
#
#   ./demo.sh            build the cluster, fill its memory, watch a third-party controller repair it
...
STEPS=(
    00-setup.sh
    01-show-cluster.sh
    02-trigger-pressure.sh
    03-watch-remediation.sh
)

usage() {
    cat <<EOF
NVSentinel local custom remediation demo
...
run_all() {
    ...
    section "NVSentinel local custom remediation demo"
    log "Build a cluster, exhaust a node's memory, and watch NVSentinel route the repair to a controller it has never heard of."
